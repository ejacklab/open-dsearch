    ...
    **kwargs
    ...
    for attempt in range(config.max_attempts):
        try:
            ...
            # - explicit retryable_exceptions overrides is_retryable
            # - otherwise use is_retryable whitelist
            explicit = config.retryable_exceptions is not None
            if not isinstance(e, config.retryable_exceptions):
                if explicit or not is_retryable(e):
                    raise
