...
use crate::models::{ModelClient, ModelSearchResult};
use uuid::Uuid;
...
impl GeminiClient {
    pub fn new(config: &crate::GeminiConfig) -> Result<Self> {
    ...
    /// Internal search method (not the trait method)
    async fn internal_search(&self, query: &str) -> Result<ModelSearchResult> {
        ...
        let search_items = if let Ok(parsed) = serde_json::from_str::<GeminiSearchResponse>(&text_response) {
            ...
            parsed.results.into_iter().map(|item| crate::models::SearchItem {
    ...
    /// Internal generate method (not the trait method)
    ...
    /// Internal analyze method (not the trait method)
...
pub async fn search_with_gemini(client: &GeminiClient, query: &str) -> Result<Vec<crate::models::SearchItem>> {
    let model_result = client.internal_search(query).await?;
    Ok(model_result.results)
}